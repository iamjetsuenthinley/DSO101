https://github.com/iamjetsuenthinley/DSO101-Continuous-Integration-and-Continuous-Development





# **DOCUMENTATION**



1\. Project Setup

!\[step1.1](JetsuenThinley\_02250350\_DSO101\_A1/Screenshots/Screenshot 2026-03-15 171418.png)

!\[step1.2](JetsuenThinley\_02250350\_DSO101\_A1\\Screenshots\\Screenshot 2026-03-15 171408.png)

!\[step1.3](JetsuenThinley\_02250350\_DSO101\_A1\\Screenshots\\Screenshot 2026-03-15 173048.png)

2\. Frontend Development

3\. Backend Development (Node.js API)

4\. Using Environment Variables

5\. Dockerising the Backend

6\. Dockerizing the Frontend

7\. Pushing Docker Images to Docker Hub

8\. Deploying Services on Render

9\. Creating render.yaml for Automated Deployment

10\. Connecting GitHub to Render (CI/CD)

11\. Continuous Deployment Test

12\. Final Deployment

